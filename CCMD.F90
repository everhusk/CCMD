...
COMMON / Properties 	/ Visc1, Visc2, zMdot, N, MaxIter, m, isolution,iDiffSteps, iErrorCheck
...
!VARY ROTOR DIMENSIONS FOR OPTIMIZATION
    ...
    D1s = r1s*2!m
    b1 = r1s - r1h
    A1=PI*(r1s**2 - r1h**2)!m^2 / Rotor inlet area with blockage
    ...
    !ROTOR INLET
            ...
            DO WHILE (ABS((C1 - last)/C1) .GT. 0.0000001)
                ...
                IF (j.GT.MaxIter) THEN
                    IF(iErrorCheck == 1) THEN
                        WRITE(*,*) "Error 1.1 - Max iteration reached at rotor inlet."
                        READ(*,*)
                    ELSEIF(iErrorCheck == 0) THEN
                        GOTO 1337
                    ENDIF
                END IF
                IF (ierr.NE.0) THEN
                    IF(iErrorCheck == 1) THEN
                        WRITE(*,*) "Error 1.2 - REFPROP (HSFLSH) error at rotor inlet:"
                        WRITE(*,*) herr
                        READ(*,*)
                    ELSEIF(iErrorCheck == 0) THEN
                        GOTO 1337
            ...
            IF (ierr.NE.0) THEN
                IF(iErrorCheck == 1) THEN
                    WRITE(*,*) "Error 1.3 - REFPROP (TRNPRP) error at rotor inlet:"
                    WRITE(*,*) herr
                    READ(*,*)
                ELSEIF(iErrorCheck == 0) THEN
                    GOTO 1337
    ...
    DO WHILE (ABS((PRr - PRrLast)/PRr) .GT. 0.0000001)
        kPR=kPR+1
        PRrLast = PRr
        
        IF (kPR.GT.MaxIter) THEN
            IF(iErrorCheck == 1) THEN
                WRITE(*,*) "Error 2.1 - Max iteration reached at PR loop."
                READ(*,*)
            ELSEIF(iErrorCheck == 0) THEN
                GOTO 1337
            END IF
        ...
        DO WHILE (ABS((Beta2 - Beta2last)/Beta2) .GT. 0.000001)
            ...
            IF (kbeta.GT.MaxIter) THEN
                IF(iErrorCheck == 1) THEN
                    WRITE(*,*) "Error 2.2 - Max iteration reached at Beta2 loop."
                    READ(*,*)
                END IF
                IF(iErrorCheck == 0) THEN
                    GOTO 1337
                END IF
            END IF
            ...
            IF (ABS(Beta2) > 80 .AND.  ABS(Beta2) <= 90) THEN
                IF(iErrorCheck == 1) THEN
                    WRITE(*,*) "Error 2.1 - Beta2 iteration is diverging"
                END IF
                Beta2 = 40!Prevents divergence
                IF(iErrorCheck == 0) THEN
                    GOTO 1337
                END IF
            END IF
        
            !Estimate of mean camber line length and equivalent number of blades
            Lb =  (DeltaZ - b2/2) + (D2 - D1)/(2*COS(Beta2*PI/180))!Sjolander
            Z = iFull + (LbSplitter/Lb)*iSplitter !Aungier
            ...
            DO WHILE (ABS((Effr-EffrLast)/Effr).GT.0.000001)
                kEFF = kEFF+1
                EffrLast = Effr
                IF (kEFF.GT.MaxIter) THEN
                    IF(iErrorCheck == 1) THEN
                        WRITE(*,*) "Error 2.3 - Max iteration reached at efficiency loop."
                        READ(*,*)
                    ELSEIF(iErrorCheck == 0) THEN
                        GOTO 1337
                    END IF
                ...
                DO WHILE (ABS((C2 - last)/C2) .GT. 0.0000001)
                    ...
                    IF (i.GT.MaxIter) THEN
                        IF(iErrorCheck == 1) THEN
                            WRITE(*,*) "Error 2.4 - Max iteration reached at continuity loop."
                            READ(*,*)
                        ELSEIF(iErrorCheck == 0) THEN
                            GOTO 1337
                        END IF
                    ...
                    IF (ierr.NE.0) THEN
                        IF(iErrorCheck == 1) THEN
                            WRITE(*,*) "Error 2.5 - REFPROP (HSFLSH) error at rotor inlet:"
                            WRITE(*,*) herr
                            READ(*,*)
                        ELSEIF(iErrorCheck == 0) THEN
                            GOTO 1337
                ...
                IF (ierr.NE.0) THEN
                    IF(iErrorCheck == 1) THEN
                        WRITE(*,*) "Error 2.6 - REFPROP (TRNPRP) error at rotor outlet:"
                        WRITE(*,*) herr
                        READ(*,*)
                    ELSEIF(iErrorCheck == 0) THEN
                        GOTO 1337
        ...
        IF (ierr.NE.0) THEN
            IF(iErrorCheck == 1) THEN
                WRITE(*,*) "Error 2.7 - REFPROP (TRNPRP) error at parasitic losses:"
                WRITE(*,*) herr
                READ(*,*)
            ELSEIF(iErrorCheck == 0) THEN
                GOTO 1337
        ...
        RecirculationWork = 0.000008*SINH(3.5*(ATAN(Cu2/Cr2))**3)*Df**2*U2**2
        ...
        IF (iDiffAnalysis == 1) THEN
            ...
            !Initial guess for next downstream location
            H = H3
            S = S3
            rho = rho3
            H0 = H03
            ...
            DO WHILE (i /= iDiffSteps)
                ...
                Cm = zMdot / (2*PI*radius*rho*b*(1-Blockage))
                    ...
                    DO WHILE (ABS((Cm-CmLastIter)/Cm).GT.0.000001)
                        ...
                        Cm = zMDot / (2*PI*radius*rho*b*(1-Blockage))
                        Slast=S
                        IF (kdiff.GT.MaxIter) THEN
                            IF(iErrorCheck == 1) THEN
                                WRITE(*,*) "Error 3.1 - Max iteration reached at diffuser step",i
                                READ(*,*)
                            ELSEIF(iErrorCheck == 0) THEN
                                GOTO 1337
                            END IF
                        ...
                        IF (ierr.NE.0) THEN
                            IF(iErrorCheck == 1) THEN
                                WRITE(*,*) "Error 3.2 - REFPROP (HSFLSH) error at diffuser step ",i,":"
                                WRITE(*,*) herr
                                READ(*,*)
                            ELSEIF(iErrorCheck == 0) THEN
                                GOTO 1337
                ...
                IF (Del /= 0) THEN
                    ...
                    IF (ierr.NE.0) THEN
                        IF(iErrorCheck == 1) THEN
                            WRITE(*,*) "Error 3.3 - REFPROP (TRNPRP) error at diffuser B.L.:"
                            WRITE(*,*) herr
                            READ(*,*)
                        ELSEIF(iErrorCheck == 0) THEN
                            GOTO 1337
...
11 FORMAT (A35,F14.7,A12) !F = Real, Field Width = 14, and 7 Decimal Places
    ...
    WRITE (10,11) "Total Density (rho02):                 ",rho02," kg/m^3"
    WRITE(10,*)
